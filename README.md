...
A Compose Multiplatform port of the WWDC-style "Hello, world!" touch distortion effect. Drag across the screen to smear text with velocity-based directional motion blur and chromatic aberration — powered by AGSL shaders, ported from the original Metal `[[stitchable]]` shader.
...
## AGSL for Android Devs — Explained Simply

If you know Compose, you already understand AGSL. Here's the mapping:
...
## Shader — Metal vs AGSL

The original Metal `[[stitchable]]` shader (by [@dankuntz](https://twitter.com/dankuntz)):
```metal
...
`compose-multiplatform` `kotlin-multiplatform` `agsl` `shader` `android` `touch-interaction` `chromatic-aberration` `motion-blur` `text-distortion` `metal-shader-port`
